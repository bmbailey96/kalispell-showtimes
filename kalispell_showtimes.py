...
# overhauled headers to mimic a real Windows Chrome browser perfectly
HEADERS = {
...
def _guess_year(today: date, month: int, day_num: int) -> int:
    ...
    for y in (today.year - 1, today.year, today.year + 1):
        try: candidates.append(date(y, month, day_num))
        except ValueError: pass
    if not candidates: return today.year
...
def parse_tribute_schedule(html: str) -> Dict[str, Dict]:
    ...
    # Update selectors: Tribute sometimes uses .media-body h2
    ...
    for h2 in headers:
        ...
        if not title or len(title) < 2 or title.lower() in ("regular showtimes", "showtimes", "coming soon"):
            continue
            ...
            for b in ticket_div.find_all("b"):
                ...
                if m:
                    ...
                    if month:
                        ...
                        try: info["dates"].add(date(year, month, day_num))
...
_cache = {"fetched_at": None, "spans": None, "fetch_error": None, "fetch_status": None, "fetch_url": None, "html_len": None}

def get_cached_spans() -> Tuple[Optional[Dict[str, Dict]], Optional[str]]:
    now = now_local()
    if _cache["fetched_at"] and (now - _cache["fetched_at"]).total_seconds() < CACHE_TTL_SECONDS and _cache["spans"]:
        return _cache["spans"], None

    html, err, status, final_url = fetch_tribute_html()
    _cache.update({"fetched_at": now, "fetch_error": err, "fetch_status": status, "fetch_url": final_url, "html_len": len(html) if html else 0})

    if not html: return None, (err or "Fetch failed")
...
def api_debug_raw():
    html, err, status, final_url = fetch_tribute_html()
    return jsonify({
        "ok": bool(html), "error": err, "status": status, "final_url": final_url,
        "len": len(html) if html else 0, "has_media_heading": ("media-heading" in html if html else False),
        "sample_head": html[:1000] if html else None
    })

@app.route("/api/showtimes")
def api_showtimes():
    try: days = int(request.args.get("days", DEFAULT_DAYS_AHEAD))
    except: days = DEFAULT_DAYS_AHEAD
    days = max(1, min(days, 365))
    spans, err = get_cached_spans()
    if err or not spans:
        return jsonify({"generated_at": now_local().isoformat(), "movies": [], "error": err, "debug": _cache}), 503
    return jsonify({"generated_at": now_local().isoformat(), "days_ahead": days, "movies": build_schedule(days, spans), "source": "TributeMovies"})

@app.route("/")
def index():
    # ... (Keep your existing HTML index string here, it works fine!)
    return "Render HTML Index Here" # Replace with your original long HTML string
